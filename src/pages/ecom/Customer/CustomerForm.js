...
const CustomerForm = React.memo(({ actions, allFormData, getFormData, submitFromOutside, services, ...props }) => {
  const [editMode, setEditMode] = useState(false);
  const [tabIndex, setTabIndex] = useState(GENERAL_TAB_INDEX);
  const [nameSelectedTab, setNameSelectedTab] = useState('');

  /** step 2 */
  const [tabIndexWithError, setTabIndexWithError] = useState({[GENERAL_TAB_INDEX]: true, [CONTACT_TAB_INDEX]: true,});
  ...
  useEffect(() => {
    ...
    if(isEditable()){
      ...
      actions.setBreadcrumbHeader([
        {title: props.intl.formatMessage({id: "Clientes.titulo", defaultMessage: "Clientes"}), href:"/ecom/clientes"},
