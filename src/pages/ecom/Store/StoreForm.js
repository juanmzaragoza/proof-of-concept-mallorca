...
const StoreForm = React.memo(({ actions, allFormData, getFormData, submitFromOutside, services, ...props }) => {
  ...
  /** step 2 */
  const [tabIndexWithError, setTabIndexWithError] = useState({[GENERAL_TAB_INDEX]: true, [CONTACT_TAB_INDEX]: false,});
  ...
  useEffect(() => {
    ...
    if(isEditable()){
      ...
      actions.setBreadcrumbHeader([
        {title: props.intl.formatMessage({id: "Almacen.titulo", defaultMessage: "Almacen"}), href:"/ecom/almacenes"},
  ...
  useEffect(()=>{
    if(isEditable()){
      ...
      actions.setBreadcrumbHeader([
        {title: props.intl.formatMessage({id: "Almacen.titulo", defaultMessage: "Almacén"}), href:"/ecom/almacenes"},
        {title: nom, href:"/ecom/almacenes"},
