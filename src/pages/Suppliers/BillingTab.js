...
const ContactTab = ({formData, setFormData, getFormData, ...props}) => {
  const [
    ...
    addValidity,
    formIsValid
  ] = useTabForm({fields: {[BILLING_DATA_SECTION_INDEX]:false}, setIsValid: props.setIsValid});
  ...
  const tabs = [
    {
      ...
      label: <FormattedMessage id={"Proveedores.Facturacion.pedidos"} defaultMessage={"Pedidos"}/>,
      key: 0,
      component: <GenericForm formComponents={ordersConfig}
                              emptyPaper={true}
                              ...
                              handleIsValid={value => addValidity(ORDERS_SECTION_TAB_INDEX,value)}
                              onBlur={(e) => handleTouched(ORDERS_SECTION_TAB_INDEX)}
                              {...props} />
    },
    {
      label: <FormattedMessage id={"Proveedores.tvencimiento"} defaultMessage={"Tipos de Vencimiento"}/>,
      key: 1,
      component: "Tipos de Proveedor"
    },
    {
      label: <FormattedMessage id={"Proveedores.Facturacion.varios"} defaultMessage={"Varios"}/>,
      key: 2,
      component: "Cajas"
    }
  ];
  ...
  return (
    <Grid container >
      <Grid xs={12} item>
        <OutlinedContainer className="contact-tab-container" title={<FormattedMessage id={"Proveedores.Facturacion.titulo"} defaultMessage={"Datos de Facturación"}/>}>
          <GenericForm
            ...
            submitFromOutside={props.submitFromOutside}
            onSubmit={() => props.onSubmitTab(formData)}
            handleIsValid={value => addValidity(BILLING_DATA_SECTION_INDEX,value)}
