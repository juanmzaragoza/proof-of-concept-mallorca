...
const LINIA_TAB_INDEX = 1;
const VARIOS_TAB_INDEX = 2;


const SuppliersOrdersForm = React.memo(
  ({
    actions,
    allFormData,
    getFormData,
    ...
    const tabs = [
      ...
      {
        ...getTranslations("PedidosProveedor.tabs.liniasPedido", "Líneas Pedido"),
        key: LINIA_TAB_INDEX,
        error: tabHasError(LINIA_TAB_INDEX),
        component: (
          <LiniasTab
            setIsValid={(value) =>
              setTabIndexWithError({
                ...tabIndexWithError,
                [LINIA_TAB_INDEX]: !value,
              })
            }
            editMode={editMode}
            getFormData={getFormData}
    ...
    useEffect(() => {
      if (isEditable()) {
        ...
        actions.setBreadcrumbHeader([
          ...
          { title: nom, href: "/fact/pedidos-proveedores" },
