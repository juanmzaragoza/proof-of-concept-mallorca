...
const DocumentFooterForm = React.memo(({ actions, allFormData, getFormData, submitFromOutside, services, ...props }) => {
  ...
  const [tabIndex, setTabIndex] = useState(DOCUMENT_FOOTER_TAB_INDEX);
  ...
  const handleSubmitTab = () => {
    ...
    if(some(Object.keys(tabIndexWithError), (index) => tabIndexWithError[index])){
      ...
      isEditable()? update(id, allFormData):create(allFormData, () => {
        goToTab(DOCUMENT_FOOTER_TAB_INDEX);
      });
    }
  }
  ...
  const tabs = [
    {
      ...getTranslations("PieDocumento.titulo","Pies Documentos"),
      key: DOCUMENT_FOOTER_TAB_INDEX,
