...
const DatosContablesTab = ({formData, setFormData, getFormData, ...props}) => {
  const [ touched, handleTouched, addValidity, formIsValid ] 
  = useTabForm({fields: {[DATOS_CONTABLES_SECTION_INDEX]: false, [INGRESOS_SECTION_TAB_INDEX]:false, 
    [PAGOS_SECTION_TAB_INDEX]:false}, setIsValid: props.setIsValid});
  ...
  const tabs = [
    {
      ...
      label: <FormattedMessage id={"DocumentosPago.ingresos"} defaultMessage={"Ingresos"}/>,
      key: 0,
      component: <GenericForm formComponents={Ingresos}
                              emptyPaper={true}
                              ...
                              handleIsValid={value => addValidity(INGRESOS_SECTION_TAB_INDEX,value)}
                              onBlur={(e) => handleTouched(INGRESOS_SECTION_TAB_INDEX)}
                              {...props} />
    },
    {
      className: "general-tab-subtab",
      label: <FormattedMessage id={"DocumentosPago.pagos"} defaultMessage={"Pagos"}/>,
      key: 1,
      component: <GenericForm formComponents={Pagos}
                              emptyPaper={true}
                              setFormData={setFormData}
                              getFormData={getFormData}
                              loading={props.loading}
  ...
  return (
    <Grid container >
      <Grid xs={12} item>
        <OutlinedContainer className="general-tab-container" title={<FormattedMessage id={"Clientes.tabs.datosContables"} defaultMessage={"Datos contables"}/>}>
          <GenericForm formComponents={createConfiguration}
                       ...
                       submitFromOutside={props.submitFromOutside}
                       onSubmit={() => props.onSubmitTab(formData)}
                       handleIsValid={value => addValidity(DATOS_CONTABLES_SECTION_INDEX,value)}
