...
const InvoiceForm = React.memo(({ actions, allFormData, getFormData, submitFromOutside, services, ...props }) => {
  const [editMode, setEditMode] = useState(false);
  const [tabIndex, setTabIndex] = useState(GENERAL_TAB_INDEX);
  const [nameSelectedTab, setNameSelectedTab] = useState('');

  const [tabIndexWithError, setTabIndexWithError] = useState({[GENERAL_TAB_INDEX]: false, [CONTAB_TAB_INDEX]: false});
  ...
  const tabs = [
    ...
    {
      ...getTranslations("Facturas.tabs.contabilidad","Contabilidad"),
  ...
  useEffect(() => {
    ...
    if(isEditable()){
      ...
      actions.setBreadcrumbHeader([
        {title: props.intl.formatMessage({id: "Facturas.titulo", defaultMessage: "Facturas"}), href:"/fact/facturas"},
