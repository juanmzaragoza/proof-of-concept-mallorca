...
const SupplierRatesForm = React.memo(
  ({
    actions,
    allFormData,
    getFormData,
    ...
    const [mostrarBasica, setMostrarBasica] = useState("");
    ...
    const tabs = [
      ...
      },
      {
        ...
        component: mostrarBasica
          ? "Pantalla para tarifa tipo Básica"
          : "Pantalla para tarifa tipo General",
      },
    ];
    ...
    useEffect(() => {
      const tipus = getFormData("tipusTarifa");

      if (tipus === "BASICA") {
        setMostrarBasica(true);
      } else {
        setMostrarBasica(false);
      }
    }, [getFormData("tipusTarifa")]);
    ...
    useEffect(() => {
      if (isEditable()) {
        ...
        actions.setBreadcrumbHeader([
          ...
          { title: desc, href: "/fact/tarifas-proveedor" },
